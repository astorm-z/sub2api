export default {
  ...
  imageGeneration: {
    title: 'AI生图',
    description: '使用你自己的 API Key 通过 Responses API 发起文生图、参考图生成或遮罩编辑请求，成功结果会在浏览器中临时保留最近 10 次。',
    endpointGenerate: '当前接口：/v1/responses',
    endpointEdit: '当前接口：/v1/responses',
    loadingKeys: '正在加载可用 API Key',
    noKeySelected: '尚未选择 API Key',
    selectedKeySummary: '{name} · {group}',
    noEligibleKeys: '当前没有可用于 AI 生图的 API Key。请先创建并绑定余额类型的 OpenAI 分组密钥。',
    lastCachedAt: '最近缓存：{time}',
    form: {
      apiKey: 'API Key',
      apiKeyPlaceholder: '请选择可用的 API Key',
      mode: '模式',
      modeGenerate: '文生图',
      modeEdit: '图生图',
      modeMask: '图生图（遮罩）',
      ...
      responseModel: 'Responses 模型',
      responseModelPlaceholder: '默认 gpt-5.5',
      imageModel: '图片模型',
      imageModelPlaceholder: '默认 gpt-image-2',
      modelSelectPlaceholder: '请选择模型',
      prompt: '提示词',
      promptPlaceholder: '例如：生成一只戴宇航员头盔的橘猫，像素插画风格，浅色背景。',
      editPromptPlaceholder: '例如：把图片里的猫改成橘色毛发，保留主体构图和背景氛围。',
      maskPromptPlaceholder: '例如：只把红色遮罩区域改成蓝色毛衣，其他区域保持不变。',
      promptHint: '建议描述主体、风格、镜头和背景，效果会更稳定。',
      editPromptHint: '改图时建议明确“保留什么、修改什么”。',
      maskPromptHint: '红色绘制区域会作为透明遮罩提交，表示希望模型修改的区域。',
      sourceImages: '源图',
      sourceImagesSingle: '源图（仅一张）',
      sourceImagesHint: '图生图会将一张或多张源图作为参考图提交。',
      maskSourceImagesHint: '遮罩模式只使用一张源图，遮罩在页面内绘制生成。',
      addImages: '添加图片',
      noSourceImages: '还没有上传原始图片',
      maskEditor: '遮罩编辑器',
      maskEditorHint: '在图片上用红色标出需要修改的区域；提交时会自动导出为 mask.png。',
      maskEditorNeedsSource: '请先上传一张原始图片，然后在这里绘制遮罩区域。',
      maskToolBrush: '笔刷',
      maskToolCircle: '圆形',
      maskToolRectangle: '矩形',
      maskToolEraser: '橡皮',
      brushSize: '笔刷大小',
      undoMask: '撤销',
      clearMask: '清空遮罩',
      previewMask: '预览遮罩',
      expandMaskEditor: '放大编辑',
      exitMaskEditorFullscreen: '退出放大',
      maskPreviewTitle: '遮罩预览：透明区域将被修改',
      size: '尺寸',
      sizeAuto: '自动',
      sizeLandscape: '1536 × 1024（横向）',
      sizePortrait: '1024 × 1536（纵向）',
      sizeSquare: '正方形',
      sizeLandscapeLabel: '横屏',
      sizePortraitLabel: '竖屏',
      sizeCustom: '自定义',
      customSize: '自定义尺寸',
      customSizeWidth: '宽度',
      customSizeHeight: '高度',
      customSizeWidthPlaceholder: '例如 1024',
      customSizeHeightPlaceholder: '例如 1024',
      customSizeHelpTitle: 'OpenAI 自定义尺寸限制',
      customSizeHelpMaxEdge: '最大边长 ≤ 3840px',
      customSizeHelpMultiple: '宽高都必须是 16px 的倍数',
      customSizeHelpRatio: '长边与短边比例 ≤ 3:1',
      customSizeHelpPixels: '总像素数必须在 655,360 到 8,294,400 之间',
      count: '数量',
      responseFormat: '返回格式',
      responseFormatBase64: 'b64_json（推荐）',
      responseFormatUrl: 'url',
      advanced: '高级参数',
      advancedHint: '常用场景可直接使用默认值，需要时再展开。',
      quality: '质量',
      qualityLow: '低',
      qualityMedium: '中',
      qualityHigh: '高',
    ...
    result: {
      ...
      cachedSummary: '已从浏览器历史记录恢复结果，共 {count} 张。',
      readySummary: '本次已生成 {count} 张图片。',
      generating: '正在生成图片',
      generatingHint: '不同模型和尺寸耗时不同，请稍等片刻。',
      failed: '生成失败',
      emptyTitle: '还没有生成结果',
      emptyDescription: '选好 Key、写好提示词后，点击“开始生成”即可。',
      copyAll: '复制全部链接',
      downloadAll: '下载全部',
      download: '下载',
      copyLink: '复制链接',
    ...
    history: {
      title: '历史记录',
      summary: '浏览器本地使用 IndexedDB 保留最近 {max} 次结果，超过后自动清理旧历史。当前 {count} 次。',
      emptyTitle: '暂无历史记录',
      emptyDescription: '成功生成后会自动临时保留最近 10 次结果。',
      imageCount: '{count} 张',
      meta: '{mode} · {responseModel} · {imageModel} · {size}',
      emptyPrompt: '未记录提示词',
      clearConfirmTitle: '清空历史记录？',
      clearConfirmMessage: '这会删除浏览器本地保存的最近生图历史，并清空当前结果和错误状态。该操作不可撤销。',
      clearConfirmButton: '确认清空',
    },
    messages: {
      generateSuccess: '已生成 {count} 张图片',
      saveGeneratedImagesSoon: '请尽快下载图片或复制链接保存。本站不会存储你的生成图片，浏览器 IndexedDB 空间也有限，历史记录只会临时保留最近 10 次结果。',
      historySaveSkipped: '图片已生成，但浏览器本地存储空间不足，本次结果未保存到历史记录。请立即下载图片或复制链接保存。',
      cancelled: '已取消本次请求',
      cacheCleared: '已清空生图历史记录',
      linksCopied: '已复制全部图片链接',
      linkCopied: '已复制图片链接',
      openedInNewTab: '当前图片已在新标签页打开',
      maskSingleSourceOnly: '遮罩编辑只支持一张原始图片，已保留第一张。',
    },
    errors: {
      loadKeysFailed: '加载 API Key 失败',
      keyRequired: '请先选择一个可用的 API Key',
      responseModelRequired: '请选择 Responses 模型',
      imageModelRequired: '请选择图片模型',
      modelsLoading: '正在加载当前 Key 支持的模型，请稍后再试',
      modelsLoadFailed: '加载当前 Key 支持的模型失败',
      noResponseModels: '当前 Key 没有可用于 Responses 的文本模型',
      noImageModels: '当前 Key 没有可用于生图的图片模型',
      promptRequired: '请输入提示词',
      sourceImagesRequired: '图生图模式至少需要上传一张源图',
      maskSourceImageRequired: '遮罩编辑模式需要上传一张原始图片',
      maskDrawingRequired: '请先在原始图片上绘制需要修改的遮罩区域',
      countRange: '数量必须是 1 到 10 之间的整数',
      outputCompressionRange: '输出压缩必须是 0 到 100 之间的整数',
      customSizeRequired: '请输入有效的自定义宽高',
      customSizeMaxEdge: 'OpenAI 限制自定义尺寸最大边长不能超过 {max}px',
      customSizeMultiple: 'OpenAI 限制自定义尺寸的宽高都必须是 {multiple}px 的倍数',
      customSizeRatio: 'OpenAI 限制自定义尺寸的长边与短边比例不能超过 {ratio}:1',
      customSizePixels: 'OpenAI 限制自定义尺寸总像素数必须在 {min} 到 {max} 之间',
      maskSourceLoadFailed: '原始图片加载失败，请重新上传后再试',
      maskNotReady: '遮罩编辑器尚未准备好',
      maskExportFailed: '导出遮罩失败',
      copyFailed: '复制失败，请手动复制链接',
      noImagesReturned: '接口已返回成功，但没有拿到可展示的图片数据',
      streamResponseRequired: '生图页面只支持流式 Responses 响应',
      unsupportedImageFormat: '仅支持 PNG、JPEG、WebP 图片',
      gptImage2TransparentUnsupported: 'gpt-image-2 不支持透明背景，请改为自动或不透明背景',
      generateFailed: '生图请求失败',
    },
  },
